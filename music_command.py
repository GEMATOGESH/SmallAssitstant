...
def init():
    ...
    name = os.getlogin()
    paths.append("C:\\Users\\" + name + "\\Music")

    exts = ["wav", "aiff", "mp3", "aac", "ogg", "wma", "flac", "m4a"]
    ...
    vc.say("Here's what I found in your music folder. Do you want anything specific?")
...
def main_choice(files):    
    while True:
        ...
        if answer == "yes":
            ...
            vc.say("Okay, I'll play a random track.")
...
def something_special(files):
    while True:
        ...
        similar = []
        for file in files:
            if music_track.lower() in file.lower():
                similar.append(file)
        ...
        if len(similar) == 1:
            ...
            vc.say("Here are the files that I think you meant, which one to play?")
            ...
            while True:
                ...
                if not number.isnumeric():
                    if number != "exit":
                        little_dude.change_body()
                        little_dude.change_face("questinable")
                        vc.say("There's no track with this number. Try again.")
                        little_dude.change_body("_headphones")
                        little_dude.change_face("none")
                        continue
